...
function Renderer(canvas, textureFromMaterialFunction) {
  getHttpRequest('/static/js/webgl/shaders.txt', this.onloadShaders.bind(this));
...
Renderer.prototype.onloadShaders = function(req) {
  ...
  // Set up program for rendering.
  var skinVsrc = shaders['shader_vertex_skin'];
  var skinFsrc = shaders['shader_fragment_skin'];
  this.skinProgram_ = new Program(gl, [vertexShader(gl, skinVsrc),
                                       fragmentShader(gl, skinFsrc)]);
...
Renderer.prototype.draw_ = function(opt_forId) {
  ...
  this.then = now;
  $('#fps-display').html("FPS: "+(1.0/this.timeSinceLastFrame).toFixed(1));

  if (!this.shadersLoaded_)
    return;

  if (this.forceColored_ || this.forceXRay) {
    opt_forId = true;  // colorcoded
  }

  if(this.forceXRay)
  {
    this.program_ = this.skinProgram_;
  }
  else if (this.forceColored_) {
  ...
  if(this.forceXRay)
  {
    this.program_.enableVertexAttribArrays(['a_position',
                                            'a_colorIndex',
                                            'a_normal']);
    this.program_.disableVertexAttribArrays(['a_texcoord']);
  }
  else if (this.forceColored_) {
    this.program_.enableVertexAttribArrays(['a_position',
                                            'a_colorIndex']);
...
Renderer.prototype.toggleAlphaOff = function()
{
    this.gl_.clearColor(0, 0, 0, 1.0);
};

Renderer.prototype.toggleAlphaOn = function()
{
    this.gl_.clearColor(0, 0, 0, 0.0);
};
