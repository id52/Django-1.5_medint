...
Mesh.prototype.free = function()
{
  var gl = this.gl_;
  if(this.texture_[0])
    gl.deleteTexture(this.texture_[0]);
  if(this.texture_[1])
    gl.deleteTexture(this.texture_[1]);
...
Mesh.prototype.bind = function(program) {
  ...
  gl.bindTexture(gl.TEXTURE_2D, this.texture_[0]);
  gl.activeTexture(gl.TEXTURE1);
  gl.uniform1i(renderer.program_.set_uniform.u_normal_sampler, 1);
  gl.bindTexture(gl.TEXTURE_2D, this.texture_[1]);
