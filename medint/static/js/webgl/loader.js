...
function downloadMesh(path, meshEntry, decodeParams, callback) {
  ...
  function onprogress(req, e) {
    while (idx < meshEntry.length) {
      ...
      meshParams.additionalParams = decodeParams.additionalParams;
...
function downloadModelJson(jsonUrl, decodeParams, callback) {
  getJsonRequest(jsonUrl, function(loaded) {
    for (var attrname in loaded.materials) { MATERIALS[attrname] = loaded.materials[attrname]; }
    loaded.decodeParams.additionalParams = decodeParams;
    downloadMeshes(jsonUrl.substr(0,jsonUrl.lastIndexOf("/")+1),
                   loaded.urls, loaded.decodeParams, callback);
  });
}
